// PayPal Create Order - Dubai Account
import { serve } from 'https://deno.land/std@0.168.0/http/server.ts';
import { createClient } from 'https://esm.sh/@supabase/supabase-js@2';

const PAYPAL_DUBAI_CLIENT_ID = Deno.env.get('PAYPAL_DUBAI_CLIENT_ID');
const PAYPAL_DUBAI_SECRET = Deno.env.get('PAYPAL_DUBAI_SECRET');
...
serve(async (req) => {
    ...
    try {
        ...
        if (!PAYPAL_DUBAI_CLIENT_ID || !PAYPAL_DUBAI_SECRET) {
            console.error('Missing PayPal credentials');
            throw new Error('Server misconfiguration: Missing PayPal credentials');
        }
        ...
        const authResponse = await fetch(`${PAYPAL_API_URL}/v1/oauth2/token`, {
            ...
            headers: {
                ...
                'Authorization': `Basic ${btoa(`${PAYPAL_DUBAI_CLIENT_ID}:${PAYPAL_DUBAI_SECRET}`)}`,
            },
